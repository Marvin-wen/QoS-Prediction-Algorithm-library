...
class UIPCCModel(object):
            ...
            for j in range(i + 1, m):
                ...
                if len(intersect) == 0 or \
                    ...
                    try:
                        ...
                        sim = (2 * len(intersect) /
                               (len(nonzero_j) +
                                len(nonzero_j))) * sim  # 使用增强的PCC计算公式
    ...
    def _get_similarity_users(self, uid, topk=-1):
        ...
        ordered_sim_uid = (-self.similarity_user_matrix[uid]
                           ).argsort()  # 按相似度从大到小排序后, 相似用户对应的索引
        ordered_sim_uid = [
            sim_uid for sim_uid in ordered_sim_uid
            if self.similarity_user_matrix[sim_uid][uid] > 0
        ]  # 只考虑相似度大于0的相似用户
        if topk == -1:
    ...
    def _get_similarity_items(self, iid, topk=-1):
        ...
        """
        assert isinstance(topk, int)
        ordered_sim_iid = (-self.similarity_item_matrix[iid]
                           ).argsort()  # 按相似度从大到小排序后, 相似项目对应的索引
        ordered_sim_iid = [
            sim_iid for sim_iid in ordered_sim_iid
            if self.similarity_item_matrix[sim_iid][iid] > 0
        ]  # 只考虑相似度大于0的相似用户
        if topk == -1:
    ...
    def _ipcc(self, uid, iid, similarity_items, i_mean):
        ...
        for sim_iid in similarity_items:  # 对于目标用户的每一个相似用户
            ...
            similarity = self.get_user_similarity(sim_iid,
                                                  uid)  # 相似用户与目标用户的相似度
    ...
    def fit(self, traid):
        ...
        """
        self.matrix = triad_to_matrix(traid, self._nan_symbol)  # 数据三元组转用户项目矩阵
        ...
        self.similarity_user_matrix, self.similarity_item_matrix = self._get_similarity_matrix(
        )  # 获取用户相似度矩阵和项目相似度矩阵
