...
class MFModel(object):
    ...
    def _init_vec(self):
        """初始化用户和项目的特征矩阵
        """
        self.user_vec = 2 * np.random.random((self.n_user, self.latent_dim)) - 1
        self.item_vec = 2 * np.random.random((self.n_item, self.latent_dim)) - 1
    ...
    def fit(self, traid, test, epochs=100, verbose=True, early_stop=True):
        ...
        for epoch in tqdm(range(epochs), desc="MF Training Epoch"):
            ...
            for row in traid:
                ...
                e_ui = y - y_pred
                # 计算用户梯度
                user_grad = -2 * e_ui * self.item_vec[item_idx] + 2 * self.lambda_ * self.user_vec[user_idx]
                # 计算项目梯度
                item_grad = -2 * e_ui * self.user_vec[user_idx] + 2 * self.lambda_ * self.item_vec[item_idx]
                # 梯度更新
            ...
            if verbose and (epoch + 1) % 10 == 0:
                ...
                print(f"[{epoch + 1}/{epochs}] MAE:{mae(y_list, y_pred_list):.5f}")

    def predict(self, traid):
        assert self.user_vec is not None, "Please fit first e.g. model.fit()"
        y_pred_list = []
        y_list = []
        for row in tqdm(traid, desc='MF Perdicting: '):
